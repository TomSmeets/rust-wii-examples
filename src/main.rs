...
fn main(_argc: isize, _argv: *const *const u8) -> isize {
    ...
    let backgroundColor: GXColor = GXColor { r:  0, g:  0, b: 0, a:  255};

    let mut camera = guVector { x: 0.0, y: 0.0, z: 0.0 };
    let mut up     = guVector { x: 0.0, y: 1.0, z: 0.0 };
    let mut look   = guVector { x: 0.0, y: 0.0, z: -1.0 };
    ...
    GX_SetViewport(0.0,0.0,(*screenMode).fbWidth as f32,(*screenMode).efbHeight as f32,0.0,1.0);
    GX_SetDispCopyYScale((*screenMode).xfbHeight as f32 / (*screenMode).efbHeight as f32 );
    GX_SetScissor(0,0,(*screenMode).fbWidth as u32,(*screenMode).efbHeight as u32);
    GX_SetDispCopySrc(0,0,(*screenMode).fbWidth,(*screenMode).efbHeight);
    GX_SetDispCopyDst((*screenMode).fbWidth,(*screenMode).xfbHeight);
    GX_SetCopyFilter((*screenMode).aa,(*screenMode).sample_pattern.as_mut_ptr(),
                     GX_TRUE as u8,(*screenMode).vfilter.as_mut_ptr());
    GX_SetFieldMode((*screenMode).field_rendering, if (*screenMode).viHeight==2*(*screenMode).xfbHeight { GX_ENABLE as u8 } else {GX_DISABLE as u8});
    ...
    GX_SetArray(GX_VA_POS , vertices.value.as_mut_ptr() as _, 3*core::mem::size_of::<i16>() as u8);
    GX_SetArray(GX_VA_CLR0 , colors.value.as_mut_ptr() as _, 4*core::mem::size_of::<u8>() as u8);
    GX_SetNumChans(1);
    GX_SetNumTexGens(0);
    GX_SetTevOrder(GX_TEVSTAGE0 as u8, GX_TEXCOORDNULL as u8, GX_TEXMAP_NULL, GX_COLOR0A0 as u8);
...
unsafe fn update_screen(mut viewMatrix: Mtx)
{
    ...
    c_guMtxTransApply(modelView.as_mut_ptr(), modelView.as_mut_ptr(), 0.0, 0.0, -50.0);
    c_guMtxConcat(viewMatrix.as_mut_ptr(),modelView.as_mut_ptr(),modelView.as_mut_ptr());
